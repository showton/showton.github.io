  ...
  var has3d,
    ...
    zoomMethods = {
      ...
      _tap: function (event) {
        ...
        var that = $(this), data = that.data().zoom, flip = data.opts.flipbook;
      ...
      _addEvent: function (eventName, event) {
        ...
        var data = this.data().zoom, time = (new Date()).getTime(), eventObject = {
          name: eventName,
          timestamp: time,
          event: event
        };
      ...
      _eventSeq: function () {
        ...
        var data = this.data().zoom, list = data.eventQueue, lastEvent = list.length - 1;
      ...
      _prepareZoom: function () {
        ...
        var flipPos, offsetLeft = 0, data = this.data().zoom, invz = 1 / this.zoom('value'), flip = data.opts.flipbook,
          dir = flip.turn('direction'), flipData = flip.data(), flipOffset = flip.offset(), thisOffset = this.offset(),
          flipSize = {height: flip.height()}, view = flip.turn('view');
        ...
        if (zoomerView != data.zoomerView) {
          ...
          for (var p = 0; p < view.length; p++) {
            ...
            var pos = flipData.pageObjs[view[p]].offset(), pageElement = $(flipData.pageObjs[view[p]]);
      ...
      zoomIn: function (event) {
        ...
        var pos, that = this, data = this.data().zoom, flip = data.opts.flipbook, zoom = data.opts.max(),
          flipOffset = flip.offset(), thisOffset = this.offset();
        ...
        var bound = zoomMethods._prepareZoom.call(this), flipPos = bound.pos,
          center = point2D(bound.size.width / 2, bound.size.height / 2), prefix = $.cssPrefix(),
          transitionEnd = $.cssTransitionEnd(), autoCenter = flip.data().opts.autoCenter;
      ...
      zoomOut: function (duration) {
        ...
        var pos, move, that = this, data = this.data().zoom, flip = data.opts.flipbook, zoom = 1,
          scaling = zoom / data.scale, prefix = $.cssPrefix(), transitionEnd = $.cssTransitionEnd(),
      ...
      flipbookWidth: function () {
        ...
        var data = this.data().zoom, flipbook = data.opts.flipbook, view = flipbook.turn('view');
      ...
      scroll: function (to, unlimited, animate) {
        ...
        var data = this.data().zoom, flip = data.opts.flipbook, flipWidth = this.zoom('flipbookWidth'),
          prefix = $.cssPrefix();
      ...
      resize: function () {
        ...
        var data = this.data().zoom, flip = data.opts.flipbook;

        if (this.zoom('value') > 1) {

          var flipOffset = flip.offset(), thisOffset = this.offset();
          ...
          if (flip.turn('display') == 'double' && flip.turn('direction') == 'ltr' && !flip.turn('view')[0]) {
            data.axis.x = data.axis.x + flip.width() / 2;
          }
      ...
      _eZoom: function () {
        ...
        var flipPos, data = this.data().zoom, flip = data.opts.flipbook, view = flip.turn('view');
      ...
      _eTurning: function (event, page, view) {
        ...
        var that = this, zoom = this.zoom('value'), data = this.data().zoom, flip = data.opts.flipbook;
      ...
      _eTurned: function (event, page) {
        ...
        if (this.zoom('value') != 1) {
          var that = this, data = this.data().zoom, flip = data.opts.flipbook;

          if (page > data.page) {
            this.zoom('scroll', point2D(0, data.scrollPos.y), false, true);
          } else if (page < data.page) {
            this.zoom('scroll', point2D(flip.width(), data.scrollPos.y), false, true);
          }
      ...
      _eMouseWheel: function (event, delta, deltaX, deltaY) {
        ...
        var data = $(this).data().zoom, cur = point2D(data.scrollPos.x + deltaX * 10, data.scrollPos.y - deltaY * 10);
      ...
      _eTouchMove: function (event) {
        ...
        var data = $(this).data().zoom, zoom = $(this).zoom('value'), flip = data.opts.flipbook,
