    ...
    turnMethods = {
      ...
      zoom: function (newZoom) {
        ...
        if (typeof(newZoom) == 'number') {
          ...
          var size = this.turn('size'), currentView = this.turn('view'), iz = 1 / data.zoom,
            newWidth = Math.round(size.width * iz * newZoom), newHeight = Math.round(size.height * iz * newZoom);
      ...
      calculateZ: function (mv) {
        ...
        var i, page, nextPage, placePage, dpage, that = this, data = this.data(), view = this.turn('view'),
          currentPage = view[0] || view[1], total = mv.length - 1, r = {
            pageZ: {},
            partZ: {},
            pageV: {}
          },
      ...
      _fold: function (point) {
        ...
        switch (data.effect) {
          ...
          case 'sheet':
            ...
            var that = this, a = 0, alpha = 0, beta, px, gradientEndPointA, gradientEndPointB, gradientStartVal,
              gradientSize, gradientOpacity, shadowVal, mv = point2D(0, 0), df = point2D(0, 0), tr = point2D(0, 0),
              folding = flipMethods._foldingPage.call(this), tan = Math.tan(alpha), ac = turnData.opts.acceleration,
              h = data.wrapper.height(), top = point.corner.substr(0, 1) == 't',
              ...
              compute = function () {
                ...
                var alpha = A90 - Math.atan2(rel.y, rel.x), gamma = alpha - Math.atan2(middle.y, middle.x),
              ...
              transform = function (tr, c, x, a) {
                ...
                var f = ['0', 'auto'], mvW = (width - h) * x[0] / 100, mvH = (height - h) * x[1] / 100, cssA = {
                    ...
                    right: f[c[2]],
                    bottom: f[c[3]]
                  }, cssB = {}, aliasingFk = (a != 90 && a != -90) ? (left ? -1 : 1) : 0,
                  origin = x[0] + '% ' + x[1] + '%';
  ...
  function gradient(obj, p0, p1, colors, numColors) {
    ...
    if (vendor == '-webkit-') {
      ...
      var dx = p1.x - p0.x, dy = p1.y - p0.y, angle = Math.atan2(dy, dx), angle2 = angle - Math.PI / 2,
        ...
        corner = point2D((p1.x < p0.x) ? obj.width() : 0, (p1.y < p0.y) ? obj.height() : 0), slope = Math.tan(angle),
        inverse = -1 / slope, x = (inverse * corner.x - corner.y - slope * p0.x + p0.y) / (inverse - slope), c = {
          x: x,
